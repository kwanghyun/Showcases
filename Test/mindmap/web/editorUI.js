...
MindMapEditor.prototype.initVisualization = function() {
	this.vis = d3.select(".editor").html('').append("svg:svg").attr("width",
			MindMapEditor.width).attr("height", MindMapEditor.height).append(
			"svg:g").attr("transform", "translate(10,0)");
}
...
MindMapEditor.prototype.registerEventHandlers = function() {
	var self = this;
	this.eventBus.registerHandler('mindMaps.event.' + self.mindMap._id,
			function(event) {
				switch (event.event) {
				case 'nodeAdded':
...
MindMapEditor.prototype.onNodeRenamed = function(event) {
	var node = findNodeByKey(this.mindMap, evnetKey);
	if (node) {
...
MindMapEditor.prototype.renderVisualization = function() {
	...
	var nodeEnter = node.enter().append("svg:g").attr("class", "node").attr(
			"opacity", "0").attr("transform", function(d) {
		return "translate(" + d.y + "," + d.x + ")";
	});
	nodeEnter.append("svg:circle").attr("r", 4.5).style("fill",
			"lightsteelblue").on("click", function(c) {
		self.addNode(c);
	});
	nodeEnter.append("svg:text").attr("x", 10).attr("dy", ".35em").text(
			function(d) {
				return d.name;
			}).on("click", function(d) {
	...
	node.exit().remove();
	var link = this.vis.selectAll("path.link").data(
			MindMapEditor.treeLayout.links(nodes), function(d) {
				return d.target.key;
			});
	link.enter().insert("svg:path", "g").attr("class", "link").attr("opacity",
			"0").attr("d", MindMapEditor.diagonalGenerator).on('click',
			function(l) {
				self.deleteNode(l.source, l.target);
			});
	link.transition().attr("d", MindMapEditor.diagonalGenerator).attr(
			"opacity", "1");
	link.exit().remove();
}