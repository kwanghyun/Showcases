DROP TABLE SmartLockerTableYearly;
CREATE EXTERNAL TABLE IF NOT EXISTS SmartLockerTableYearly 
...
DROP TABLE CourierDropOffSiteBankVsAllYearly;	
CREATE EXTERNAL TABLE IF NOT EXISTS CourierDropOffSiteBankVsAllYearly(avgDepositTime STRING, depositCount INT, overallAvgDepositTime STRING, overallDepositCount INT, 
...
'CREATE TABLE kpi_courier_dropoff_site_bank_vs_all_yearly(avgDepositTime DECIMAL(6,1), depositCount INT, OverallAvgDepositTime DECIMAL(6,1), OverallDepositCount INT, 
						transactionDate TIMESTAMP, site VARCHAR(100), bank VARCHAR(100))' );

insert overwrite table CourierDropOffSiteBankVsAllYearly 
...
FROM(
	...
	FROM SmartLockerTableYearly
	WHERE eventType = 32 AND successflag = 1 AND version = '1.0.0' AND substr(eventTime,0,4) = substr(${hiveconf:TARGET_DAY},0,4)
...
DROP TABLE CourierPickupSiteBankVsAllYearly;	
CREATE EXTERNAL TABLE IF NOT EXISTS CourierPickupSiteBankVsAllYearly(avgPickupTime STRING, pickupCount INT, overallAvgPickupTime STRING, overallPickupCount INT, 
				transactionDate TIMESTAMP, site STRING, bank STRING
) STORED BY 
'org.wso2.carbon.hadoop.hive.jdbc.storage.JDBCStorageHandler' 
TBLPROPERTIES ( 
'wso2.carbon.datasource.name'='MYSQL_DATA_SOURCE',
'hive.jdbc.table.create.query' = 
'CREATE TABLE kpi_courier_pickup_site_bank_vs_all_yearly(avgPickupTime DECIMAL(6,1), pickupCount INT, overallAvgPickupTime DECIMAL(6,1), overallPickupCount INT, 
				transactionDate TIMESTAMP, site VARCHAR(100), bank VARCHAR(100))' );

insert overwrite table CourierPickupSiteBankVsAllYearly
...
FROM(
	...
	FROM SmartLockerTableYearly
	WHERE eventType = 33 AND successflag = 1 AND version = '1.0.0' AND substr(eventTime,0,4) = substr(${hiveconf:TARGET_DAY},0,4)
	GROUP BY site, bank) s1 
JOIN (
...
DROP TABLE CustomerDropOffSiteBankVsAllYearly;	
CREATE EXTERNAL TABLE IF NOT EXISTS CustomerDropOffSiteBankVsAllYearly(avgDepositTime STRING, depositCount INT, overallAvgDepositTime STRING, overallDepositCount INT, transactionDate TIMESTAMP, site STRING, bank STRING
) STORED BY 
'org.wso2.carbon.hadoop.hive.jdbc.storage.JDBCStorageHandler' 
TBLPROPERTIES ( 
'wso2.carbon.datasource.name'='MYSQL_DATA_SOURCE',
'hive.jdbc.table.create.query' = 
'CREATE TABLE kpi_customer_drop_off_site_bank_vs_all_yearly(avgDepositTime DECIMAL(6,1), depositCount INT, OverallAvgDepositTime DECIMAL(6,1), OverallDepositCount INT, transactionDate TIMESTAMP, site VARCHAR(100), bank VARCHAR(100))' );

insert overwrite table CustomerDropOffSiteBankVsAllYearly
...
FROM(
	SELECT avg(totalTime) as totalTime, count(*) as depositCount, site, bank, ${hiveconf:TARGET_DAY} as targetDay
	FROM SmartLockerTableYearly
	WHERE eventType = 34 AND successflag = 1 AND version = '1.0.0' AND substr(eventTime,0,4) = substr(${hiveconf:TARGET_DAY},0,4)
	GROUP BY site, bank ) s1 
JOIN (
	SELECT avg(totalTime) as totalTime, count(*) as depositCount, ${hiveconf:TARGET_DAY} as targetDay
	FROM SmartLockerTableYearly
...
DROP TABLE CustomerPickupSiteBankVsAllYearly;	
CREATE EXTERNAL TABLE IF NOT EXISTS CustomerPickupSiteBankVsAllYearly(avgPickupTime STRING, pickupCount INT, overallAvgPickupTime STRING, overallPickupCount INT, transactionDate TIMESTAMP, site STRING, bank STRING
) STORED BY 
'org.wso2.carbon.hadoop.hive.jdbc.storage.JDBCStorageHandler' 
TBLPROPERTIES ( 
'wso2.carbon.datasource.name'='MYSQL_DATA_SOURCE',
'hive.jdbc.table.create.query' = 
'CREATE TABLE kpi_customer_pickup_site_bank_vs_all_yearly(avgPickupTime DECIMAL(6,1), pickupCount INT, overallAvgPickupTime DECIMAL(6,1), overallPickupCount INT, transactionDate TIMESTAMP, site VARCHAR(100), bank VARCHAR(100))' );

insert overwrite table CustomerPickupSiteBankVsAllYearly  
...
FROM(
	SELECT avg(totalTime) as totalTime, count(*) as retrieveCount, site, bank, ${hiveconf:TARGET_DAY} as targetDay
	FROM SmartLockerTableYearly
	WHERE eventType = 35 AND successflag = 1 AND version = '1.0.0' AND substr(eventTime,0,4) = substr(${hiveconf:TARGET_DAY},0,4)
	GROUP BY site, bank) s1 
JOIN (
	SELECT avg(totalTime) as totalTime, count(*) as retrieveCount, ${hiveconf:TARGET_DAY} as targetDay
	FROM SmartLockerTableYearly
...
DROP TABLE CustomerSatisfactionSurveySiteBankVsAllYearly;
CREATE EXTERNAL TABLE IF NOT EXISTS CustomerSatisfactionSurveySiteBankVsAllYearly(satisfactionRating FLOAT, OverallSatisfactionRating FLOAT, activityType STRING, transactionDate TIMESTAMP, site STRING, bank STRING
) STORED BY 
'org.wso2.carbon.hadoop.hive.jdbc.storage.JDBCStorageHandler' 
TBLPROPERTIES ( 
'wso2.carbon.datasource.name'='MYSQL_DATA_SOURCE',
'hive.jdbc.table.create.query' = 
'CREATE TABLE kpi_customer_satisfaction_survey_site_bank_vs_all_yearly(satisfactionRating DECIMAL(6,3), overallSatisfactionRating DECIMAL(6,3), activityType VARCHAR(100), transactionDate TIMESTAMP, site VARCHAR(100), bank VARCHAR(100))' );

insert overwrite table CustomerSatisfactionSurveySiteBankVsAllYearly    
...
from (
	...
	from SmartLockerTableYearly 
	where  substr(eventTime,0,4) = substr(${hiveconf:TARGET_DAY},0,4) AND successflag = 1 AND surveyScore > 0 
...
DROP TABLE LockerUtilizationPCYearly;
CREATE EXTERNAL TABLE IF NOT EXISTS LockerUtilizationPCYearly(avgLockerUtilizationPC FLOAT, avgTotalLockers FLOAT, avgOccupiedLockers FLOAT, 
...
'wso2.carbon.datasource.name'='MYSQL_DATA_SOURCE',
'hive.jdbc.table.create.query' = 
'CREATE TABLE kpi_capcity_locker_utilization_site_bank_vs_all_yearly(avgLockerUtilizationPC DECIMAL(6,1), avgTotalLockers DECIMAL(6,1), avgOccupiedLockers DECIMAL(6,1), 
...
insert overwrite table LockerUtilizationPCYearly 
...
from (
	...
	from (
		...
		from SmartLockerTableYearly 
		where substr(eventTime,0,4) = substr(${hiveconf:TARGET_DAY},0,4) AND successflag = 1 AND version = '1.0.0' 
...
DROP TABLE CapacityUtilizationRetainedTimeYearly;	
CREATE EXTERNAL TABLE IF NOT EXISTS CapacityUtilizationRetainedTimeYearly(avgRetainedTime STRING, overallAvgRetainedTime STRING, transactionDate TIMESTAMP, site STRING, bank STRING
	) STORED BY 
	'org.wso2.carbon.hadoop.hive.jdbc.storage.JDBCStorageHandler' 
	TBLPROPERTIES ( 
    'wso2.carbon.datasource.name'='MYSQL_DATA_SOURCE',
	'hive.jdbc.table.create.query' = 
	'CREATE TABLE kpi_capacity_utilization_retained_period_vs_all_yearly(avgRetainedTime DECIMAL(6,1), overallAvgRetainedTime DECIMAL(6,1), transactionDate TIMESTAMP, site VARCHAR(100), bank VARCHAR(100))' );
                                    
insert overwrite table CapacityUtilizationRetainedTimeYearly
...
FROM(
	...
	WHERE successflag = 1 AND version = '1.0.0' AND (eventType = 33 OR eventType = 35) 
		AND substr(eventTime,0,4) = substr(${hiveconf:TARGET_DAY},0,4)
