 ...
 * the party. We can only ask questions like “does A know B? “. Find the
...
public class CelebrityProblem {
	 ...
	 * edges of the graph for every possible pair [i, j]. We have NC2 pairs. If
	 * celebrity is present in the party, we will have one sink node in the
	 * graph with outdegree of zero, and indegree of N-1. We can find the sink
	 * node in (N) time, but the overall complexity is O(N2) as we need to
	 * construct the graph first. Method 2 (Recursion)
	 * 
	 ...
	 * doesn’t know N. Solve the problem of smaller instance during divide step.
	 * On the way back, we find the celebrity (if present) from the smaller
	 * instance. During combine stage, check whether the returned celebrity is
	 * known to everyone and he doesn’t know anyone. The recurrence of the
	 * recursive decomposition is,
	 * 
	 * T(N) = T(N-1) + O(N)
	 * 
	 * T(N) = O(N2). You may try writing pseudo code to check your recursion
	 * skills.
	 * 
	 * Method 3 (Using Stack)
	 * 
	 * The graph construction takes O(N2) time, it is similar to brute force
	 * search. In case of recursion, we reduce the problem instance by not more
	 * than one, and also combine step may examine M-1 persons (M – instance
	 * size). We have following observation based on elimination technique
	 ...
	 * doesn’t have acquaintance with anyone else. We will discard N elements
	 * utmost (Why?). If the celebrity is present in the party, we will call
