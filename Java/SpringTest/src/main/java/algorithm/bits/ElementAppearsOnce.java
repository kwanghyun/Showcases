...
/**
 * Method to find the element that occurs only once in the input array while
 * all the other numbers occur N times.
 * 
 * @param arr
 *            is input array.
 * @param N
 *            is number of times all elements, except one, are occurring in
 *            the input array.
...
public int findRequiredNum(int arr[], int N) {
	...
	for (int i = 0; i < arr.length; i++) {
		...
		for (int k = 0; k < 32; k++) {
			int kthSetBit = 1 << k;
			// If the kth bit is set, then increment the count of
			// countSetBit[k].
