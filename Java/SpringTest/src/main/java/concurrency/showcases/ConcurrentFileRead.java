...
public class ConcurrentFileRead {
	...
	public ConcurrentFileRead(File file) {
		this.file = file;
	}
	...
	public void processAll(int noOfThreads, int chunkSize) throws Exception {
		int count = (int) ((file.length()) / chunkSize) + 1;
		List<Callable<String>> tasks = 
				new ArrayList<Callable<String>>(count);
		
		for (int i = 0; i < count; i++)
			tasks.add(processPartTask(i * chunkSize,
					Math.min(file.length(), (i + 1) * chunkSize)));
	...
	public static void main(String argv[]) throws Exception {
		ConcurrentFileRead s = new ConcurrentFileRead(new File(argv[0]));
		s.processAll(8, 1000);
	}
}