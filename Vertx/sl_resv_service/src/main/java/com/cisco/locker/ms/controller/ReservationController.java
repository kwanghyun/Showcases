...
public class ReservationController extends AbstractVertxController {
	...
	public ReservationController(Router router, MongoClient mongo) {
		super(router, mongo);
	}
	...
	public void loadRoutes() {
		...
		router.post(Properties.API_ROOT + API_NAME).handler(ctx -> {
			...
			logger.info("newReservation :: " + newJsonObj);
			...
			mongo.findOne(COLLECTION_NAME, new JsonObject().put("packageId", newJsonObj.getString("packageId")),
				...
				if (jsonObj != null) {
					logger.error("Already exists :: " + jsonObj);
					// already exists
					ctx.fail(500);
				} else {

					LocalDateTime currDate = LocalDateTime.now();
					newJsonObj.put("reservationDate", currDate.toString()).put("expiryDate",
							TimeUtils.addDays(currDate, Properties.EXPIRY_PERIOD).toString());
...
//							? reservation.getString("expiryDate") : update.getString("expiryDate"));
	...
	private void loadData(MongoClient db) {
		db.dropCollection(COLLECTION_NAME, drop -> {
			if (drop.failed()) {
				throw new RuntimeException(drop.cause());
			}

			List<JsonObject> reservations = new LinkedList<>();
			LocalDateTime currDate = LocalDateTime.now();

			reservations.add(new JsonObject().put("site", "site1").put("bank", "bank1").put("size", "M")
					.put("packageId", "P111111").put("reservationDate", currDate.toString())
					.put("expiryDate", TimeUtils.addDays(currDate, Properties.EXPIRY_PERIOD).toString()));

			reservations.add(new JsonObject().put("site", "site2").put("bank", "bank2").put("size", "S")
					.put("packageId", "P222222").put("reservationDate", currDate.plusMinutes(5).toString())
					.put("expiryDate", TimeUtils.addDays(currDate, Properties.EXPIRY_PERIOD).toString()));

			for (JsonObject reservation : reservations) {
				db.insert(COLLECTION_NAME, reservation, res -> {
					logger.info("inserted " + reservation.encodePrettily());
				});
			}
		});
	}
