...
var CommandInterpreterManager = function() {
	var _interpreters = new Object();


	function initMetaPropertyDefinition() {
		...
		setCommandInterpreter(META.COMMAND.OPEN.key,
	...
	function setCommandInterpreter(key, interpreter) {
		debug("Setting Command interpreter for key " + key + " interpreter " + interpreter);
		_interpreters[key] = interpreter;
	}

	this.getCommandInterpreter = function(type, key) {
        debug("Getting command interpreter for type " + type + " and key " + key);		
		if(type === "request"){
	...
	initMetaPropertyDefinition();
    console.log("Initialized Command Interpreter Manager");
};
...
var CommandInterpreter = function (reqCmd, resCmd){
	...
	function convertStringToMetaDefinition(metaInfoRef, value){
	...
	function deviceWrite(str){
		str = str + getBCC(str) + 'CR';
	...
	this.interpretRequest = function(reqeust) {
		debug("[requestInterperter()::##START##]  action : " + reqeust.action);
		
		var metaInfo = reqCmd.contents;
		...
		for ( var index in metaInfo) {
			...
			if (metaInfo[index].ref != null) {
				...
				if (value.length < metaInfo[index].length){
					value = pad(value, metaInfo[index].length);
	...
	this.interpretResponse = function(responesStr) {
		...
		for ( var index in metaInfo) {	
			...
			if (metaInfo[index].ref != null) {
				value = convertStringToMetaDefinition(metaInfo[index].ref, value);
			}
			...
			if (index == metaInfo.length - 1) {
				if( endIndex == responesStr.length){
					resultStr += '"' + id + '" : "' + value + '" }';	
				}else if(resCmd.repeatableContents != null){
					...
					while(endIndex < responesStr.length){
						if(command.repeatableContents.length == 1){
							...
							value = convertStringToMetaDefinition(command.repeatableContents[0].ref,
									responesStr.substring(startIndex, endIndex));
							idx ++;
							
						}
						else if (resCmd.repeatableContents.length > 1
								&& id == META.LOCKERS_STATUS.id) {
	...
	//							debug("[handleMultiResponse()]::endIndex: " + startIndex +", endIndex : " + endIndex);
								...
								tempStr += '"'
										+ META.DOOR.id
										+ '" : "'
										+ convertStringToMetaDefinition(META.DOOR, strChunk
												.substring(0, 1)) + '" ,';
								tempStr += '"'
										+ META.PACKAGE.id
										+ '" : "'
										+ convertStringToMetaDefinition(META.PACKAGE, strChunk
												.substring(1, 2)) + '" ,';
								tempStr += '"'
										+ META.ALARM.STATUS.id
										+ '" : "'
										+ convertStringToMetaDefinition(META.ALARM.STATUS, strChunk
												.substring(2, 3)) + '" },';
			...
			if(id == "response" && isError(value) == true){
