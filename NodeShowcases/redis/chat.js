...
/*
 * HSET : Sets field in the hash stored at key to value. If key does not exist,
 * a new key holding a hash is created. If field already exists in the hash, it
 * is overwritten.
 * 
 * ZADD : Adds all the specified members with the specified scores to the sorted
 * set stored at key.
 * 
 * ZREVRANGEBYSCORE : Returns all the elements in the sorted set at key with a
...
exports.addUser = function addUser(user, name, type) {
	client.multi().hset('user:' + user, 'name', name).hset('user:' + user,
			'type', type).zadd('users', Date.now(), user).exec();
};
...
exports.addChat = function addChat(chat) {
	client.multi().zadd('rooms:' + chat.room + ':chats', Date.now(),
			JSON.stringify(chat)).zadd('users', Date.now(), chat.user.id).zadd(
			'rooms', Date.now(), chat.room).exec();
};
...
exports.addUserToRoom = function addUserToRoom(user, room) {
	client.multi().zadd('rooms:' + room, Date.now(), user).zadd('users',
			Date.now(), user).zadd('rooms', Date.now(), room).set(
			'user:' + user + ':room', room).exec();
}

exports.removeUserFromRoom = function removeUserFromRoom(user, room) {
	client.multi().zrem('rooms:' + room, user).del('user:' + user + ':room')
			.exec();
};
